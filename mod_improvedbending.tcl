...
#Modified version to give the sign of bending angle
#######################
#For the normalized vectors,calculate the angle between v1 v2 and central
#If theta1 > theta2 return 180-theta
#If theta1 < theta2 and if theta > theta2 return 180-theta otherwise 180+theta
#For both polar and azimuthal
#5/3/2015
#Han Wen


#Modified to improve the algorithm, using the plane formed by total helix
#and com instead of a simple vertical one.
#5/3/2015
#Han Wen 
...
#The logic here is to find the projection along the plane defined by total_sel and z axis.
...
set selection [atomselect top "resid 656 to 670 and segname PROC and name CA"]
set selection1 [atomselect top "resid 674 to 686 and segname PROC and name CA"]
set total_sel [atomselect top "resid 656 to 686 and segname PROC and name CA"]
#set selection [atomselect top "all and name CA"]
#modify for your choice
set mol_id [$selection molindex]
set angle_out [open "atestC" w]
...
#Loop for all frames
    ...
    #puts $principal_axis
    #puts [show $eigenvalue]
    set xt_v [lindex $I_matrix 5]
    set yt_v [lindex $I_matrix 8]
    set zt_v [lindex $I_matrix 11]
    ...
    #now have the total principal vector, do cross product vt c*(x,y,0) as normp
    set xnp [expr {-$zt_v * $cy}]
    set ynp [expr {$zt_v * $cx}]
    set znp [expr {$xt_v*$cy - $yt_v*$cx}]
    #The normal vector for polar plane will be (xnp,ynp,znp)
    #The normal vector for azimuthal plane will be (cx,cy,0)

    #modify p(5/3) with -cy to xnp, cx to ynp, adding znp 
    #Now calculate the projection on both plane:
    #V_projec =n*(V dot* n)/(n dot* n)     
    set para1_p [expr {($x1_v*($xnp) + $y1_v*($ynp) +$z1_v*($znp))/($xnp*$xnp + $ynp*$ynp + $znp*$znp)}]
    set para2_p [expr {($x2_v*($xnp) + $y2_v*($ynp) +$z2_v*($znp))/($xnp*$xnp + $ynp*$ynp + $znp*$znp)}]
    set para1_a [expr {($x1_v*($cx) + $y1_v*($cy))/($cx*$cx + $cy*$cy)}]
    set para2_a [expr {($x2_v*($cx) + $y2_v*($cy))/($cx*$cx + $cy*$cy)}]
    
    set x1_pn [expr {$para1_p*($xnp)}] 
    set x2_pn [expr {$para2_p*($xnp)}]
    set y1_pn [expr {$para1_p*$ynp}]
    set y2_pn [expr {$para2_p*$ynp}]
    set z1_pn [expr {$para1_p*$znp}]
    set z2_pn [expr {$para2_p*$znp}]
    ...
    set z1_p [expr {$z1_v - $z1_pn}]
 
    set x2_p [expr {$x2_v - $x2_pn}]
    set y2_p [expr {$y2_v - $y2_pn}]
    set z2_p [expr {$z2_v - $z2_pn}]
    ...
    #Now determine the sign of theta_p/a:
    #determine theta1 theta2:
    set cos_theta_p1 [expr { ($z1_pnor*$z1_pnor-1 + ( ($znp*$znp*$z1_pnor*$z1_pnor)/($xnp*$xnp)) )/( ($ynp*$ynp)/($xnp*$xnp)+1) + ($z1_pnor*$z1_pnor-1 + ( ($znp*$znp*$z1_pnor*$z1_pnor)/($ynp*$ynp) ))/( ($xnp*$xnp)/($ynp*$ynp)+1) + $z1_pnor*$z1_pnor }]
    set cos_theta_p2 [expr { ($z2_pnor*$z2_pnor-1 + ( ($znp*$znp*$z2_pnor*$z2_pnor)/($xnp*$xnp)) )/( ($ynp*$ynp)/($xnp*$xnp)+1) + ($z2_pnor*$z2_pnor-1 + ( ($znp*$znp*$z2_pnor*$z2_pnor)/($ynp*$ynp) ))/( ($xnp*$xnp)/($ynp*$ynp)+1) + $z2_pnor*$z2_pnor }]


    set cos_theta_a1 [expr { ($z1_anor*$z1_anor-1)/($cx*$cx/$cy*$cy+1) + ($z1_anor*$z1_anor-1)/($cy*$cy/$cx*$cx+1) + $z1_anor*$z1_anor }]
    set cos_theta_a2 [expr { ($z2_anor*$z2_anor-1)/($cx*$cx/$cy*$cy+1) + ($z2_anor*$z2_anor-1)/($cy*$cy/$cx*$cx+1) + $z2_anor*$z2_anor }]
    #puts "$cos_theta_p1, $cos_theta_p2, $cos_theta_a1,$cos_theta_a2, $xnp,$ynp,$znp"
    set theta_p1 [expr {180 * acos($cos_theta_p1) / 3.1415926}]
    set theta_p2 [expr {180 * acos($cos_theta_p2) / 3.1415926}]
    set theta_a1 [expr {180 * acos($cos_theta_a1) / 3.1415926}]
    set theta_a2 [expr {180 * acos($cos_theta_a2) / 3.1415926}]
    ...
    #puts $angle_out "$x1_pnor,$y1_pnor,$z1_pnor,1p"
    #puts $angle_out "$x2_pnor,$y2_pnor,$z2_pnor,2p"
    #puts $angle_out "$x1_anor,$y1_anor,$z1_anor,1a"
    #puts $angle_out "$x2_anor,$y2_anor,$z2_anor,2a"
